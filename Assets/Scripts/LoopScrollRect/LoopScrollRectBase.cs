...
{
    [Serializable]
    public class LoopScrollEvent
    {
        private Func<int, RectTransform> action = null;

        public void AddListener(Func<int, RectTransform> call)
        {
            action = call;
        }
        public RectTransform Invoke(int index)
        {
            if (action != null)
                return action(index);
            return null;
    ...
    public abstract class LoopScrollRectBase : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        /// 更新item的数据事件
        /// </summary>
        [FormerlySerializedAs("onItemUpdate")] public UnityEvent<Transform, int> OnItemUpdate = new();
        /// <summary>
        /// 获取Item的Transform的事件
        /// </summary>
        [FormerlySerializedAs("onItemGive")] public LoopScrollEvent OnItemGive = new();
        /// <summary>
        /// 交还Item的Transform的事件
        /// </summary>
        [FormerlySerializedAs("onItemReturn")] public UnityEvent<Transform> OnItemReturn = new();
        /// <summary>
        /// 触发滚动的事件
        /// </summary>
        [FormerlySerializedAs("onDragBegin")] public UnityEvent OnDragBegin = new();

        /// <summary>
        /// The scroll's total count for items with id in [0, totalCount]. Negative value like -1 means infinite items.
        /// </summary>
        [FormerlySerializedAs("totalCount")] [Tooltip("Total count, negative means INFINITE mode")]
        ...
        /// </summary>
        [FormerlySerializedAs("reverseDirection")] [Tooltip("Reverse direction for dragging")]
        ...
        protected float contentSpacing
        {
            get
            {
                ...
                if (m_Content != null)
                {
                    ...
                    if (gridLayout != null)
                    {
                        m_ContentSpacing = GetAbsDimension(gridLayout.spacing);
                        m_ContentLeftPadding = gridLayout.padding.left;
                        m_ContentRightPadding = gridLayout.padding.right;
                        m_ContentTopPadding = gridLayout.padding.top;
                        m_ContentBottomPadding = gridLayout.padding.bottom;
                    }
                }
                return m_ContentSpacing;
            }
        ...
        const string HINT_MSG = "循环滚动组件({0})的{1}的<color=yellow>{2}</color>值应为<color=yellow>{3}</color>";
        protected override void Awake()
        {
            ...
            if (Application.isPlaying)
            {
                ...
                var pivotV = GetAbsDimension(m_Content.pivot);
                if (pivotV != value)
                    Debug.LogError(string.Format(HINT_MSG, gameObject.name, $"容器({m_Content.name})的中心点(pivot)", pivotV, value));
                var anchorMinV = GetAbsDimension(m_Content.anchorMin);
                if (anchorMinV != value)
                    Debug.LogError(string.Format(HINT_MSG, gameObject.name, $"容器({m_Content.name})的最小锚点(anchorMin)", anchorMinV, value));
                var anchorMaxV = GetAbsDimension(m_Content.anchorMax);
                if (anchorMaxV != value)
                    Debug.LogError(string.Format(HINT_MSG, gameObject.name, $"容器({m_Content.name})的最大锚点(anchorMax)", anchorMaxV, value));
                if (direction == LoopScrollRectDirection.Vertical)
                {
                    if (!m_Vertical || m_Horizontal)
                        Debug.LogError(string.Format(HINT_MSG, gameObject.name, "滚动方向(Vertical/Horizontal)", $"{m_Vertical}/{m_Horizontal}", "True/False"));
                }
                else
                {
                    if (m_Vertical || !m_Horizontal)
                        Debug.LogError(string.Format(HINT_MSG, gameObject.name, "滚动方向(Vertical/Horizontal)", $"{m_Vertical}/{m_Horizontal}", "False/True"));
                }
            }
        }
        #endif
        ...
        public void ScrollToCell(int index, float speed)
        {
            ...
            if (speed <= 0)
            {
                //当滑动到最后一个格子时，从后向前进行填充，防止最后一个格子显示不全
                if (index == TotalCount - 1)
                    //todo RefillCellsFromEnd会导致最后一行消失，具体问题待定位，先临时全部调用RefillCells
                    RefillCells(index);//RefillCellsFromEnd();
                else
                    RefillCells(index);
                return;
            }
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                for (int i = 0; i < m_Content.childCount; i++)
                {
                    ...
                    {
                        OnItemUpdate.Invoke(m_Content.GetChild(i), itemTypeEnd);
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        protected override void OnDestroy()
        {
            ...
            OnItemGive.RemoveAllListeners();
            OnItemReturn.RemoveAllListeners();
            OnItemUpdate.RemoveAllListeners();
        }
    }
}